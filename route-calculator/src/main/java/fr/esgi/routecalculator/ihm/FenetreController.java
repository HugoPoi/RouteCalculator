...
public class FenetreController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource().equals(vue.recherche)){
			/*
			 * Les variables �  utiliser dans les recherches
			 */
			...
			if(entreeValide(depart,name,table,CN)){
				if(entreeValide(arrive,name,table,CN)){
					if(date.before(now) || date.equals(now)){
						//TODO
						//impl�ment� la methode de recherche
					}
					else{
						vue.erreur("La date est erronn�e");
					}
				}
				else{
					vue.erreur("L'arriv�e est erronn�e");
				}
			}
			else{
				vue.erreur("Le depart est erronn�e");
			}
			
			
			if(date.before(new Date()) || date.equals(new Date())){
				vue.erreur("Le choix de la date est erronn�e");
			} else{
				/*
				 * Recuperation des info entr�es dans l'IHM
				 */
			...
			/* public void lancerRequete()
			  try
			  {
			    Class.forName("com.mysql.jdbc.Driver" );                                               //Initialisation du pilote de base de donn�es
			    Connection con = DriverManager.getConnection("jdbc:mysql://database", "user", "pwd" ); //Connexion � ta base de donn�es
			    Statement instruction = con.createStatement();                                         //Cr�ation du statement
			 
			    String maRequete = "select designation_produit, type_produit FROM produit WHERE ref_produit 5";                                                                 // Entre ta requete ici
			    ResultSet resultat = instruction.executeQuery(maRequete);                              //Ex�cution de ta requete avec retour de resultat
